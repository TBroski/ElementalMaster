...
public class FiriumTreeStructure extends ElementalmasterModElements.ModElement {
	public FiriumTreeStructure(ElementalmasterModElements instance) {
		super(instance, 531);
	}
		...
		Feature<NoFeatureConfig> feature = new Feature<NoFeatureConfig>(NoFeatureConfig::deserialize) {
			...
			public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if (!dimensionCriteria)
					return false;
				if ((random.nextInt(1000000) + 1) <= 725000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("elementalmaster", "firiumtree"));
						if (template == null)
							return false;
