...
import net.bukkit.elementalmaster.item.DarkQuartzItem;
...
public class DarkQuartzOreBlock extends ElementalmasterModElements.ModElement {
	@ObjectHolder("elementalmaster:dark_quartz_ore")
	public static final Block block = null;
	public DarkQuartzOreBlock(ElementalmasterModElements instance) {
		super(instance, 63);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("dark_quartz_ore", "dark_quartz_ore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.NETHERRACK.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 8)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(15, 6, 6, 122))));
		}
	}
}
